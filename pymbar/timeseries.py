...
# TODO: Should we raise whis warning?
LONG_WARNING = ("Warning on use of the timeseries module: If the inherent timescales of the system "
               "are long compared to those being analyzed, this statistical inefficiency may be an "
               "underestimate. The estimate presumes the use of many statistically independent samples. "
               "Tests should be performed to assess whether this condition is satisfied. "
               "Be cautious in the interpretation of the data.")
LongWarning = LONG_WARNING
...
statisticalInefficiency = _deprecate(statistical_inefficiency, "statisticalInefficiency")
...
statisticalInefficiencyMultiple = _deprecate(statistical_inefficiency_multiple, "statisticalInefficiencyMultiple")
...
integratedAutocorrelationTime = _deprecate(integrated_autocorrelation_time, "integratedAutocorrelationTime")

#=============================================================================================
def integrated_autocorrelation_time_multiple(A_kn, fast=False):
...
integratedAutocorrelationTimeMultiple = _deprecate(integrated_autocorrelation_time_multiple, "integratedAutocorrelationTimeMultiple")
...
normalizedFluctuationCorrelationFunction = _deprecate(normalized_fluctuation_correlation_function, "normalizedFluctuationCorrelationFunction")

#=============================================================================================
def normalized_fluctuation_correlation_function_multiple(A_kn, B_kn=None, N_max=None, norm=True, truncate=False):
...
normalizedFluctuationCorrelationFunctionMultiple = _deprecate(normalized_fluctuation_correlation_function_multiple, "normalizedFluctuationCorrelationFunctionMultiple")
...
subsampleCorrelatedData = _deprecate(subsample_correlated_data, "subsampleCorrelatedData")
...
detectEquilibration = _deprecate(detect_equilibration, "detectEquilibration")

def statistical_inefficiency_fft(A_n, mintime=3, memsafe=None):
    """Compute the (cross) statistical inefficiency of (two) timeseries.
...
statisticalInefficiency_fft = _deprecate(statistical_inefficiency_fft, "statisticalInefficiency_fft")
...
def detect_equilibration_binary_search(A_t, bs_nodes=10):
    ...
    while True:
        ...
        for k, t in enumerate(time_grid):
            if t < T-1:
                g_t[k] = statistical_inefficiency_fft(A_t[t:], memsafe=True)
...
detectEquilibration_binary_search = _deprecate(detect_equilibration_binary_search, "detectEquilibration_binary_search")
