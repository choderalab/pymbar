...
class MBAR:
  ...
  * Example 1. Computation of relative free energies from an alchemical simulation.
  ...
  u_kln = zeros([K, L, T_k.max()], numpy.float64) # u_kln[k,l,n] will store the reduced potential energy at state l of uncorrelated snapshot n \in 1..N_k[k] from state k.
  for k in range(0,K):
  ...
  def __init__(self, u_kln, N_k, maximum_iterations=10000, relative_tolerance=1.0e-7, verbose=False, initial_f_k=None, method='adaptive', use_optimized=None, newton_first_gamma = 0.1,  newton_self_consistent = 2, maxrange = 1.0e5, initialize='zeros'):
    ...
    REQUIRED ARGUMENTS
      u_kln (KxLxNmax float array) - u_kln[k,l,n] is the reduced potential energy of uncorrelated configuration n sampled from state k, evaluated at state l.  K >= L
    ...
    if verbose: print "K = (states with samples) %d, L (total states) = %d, N_max = %d, total samples = %d" % (K, L, N_max, self.N_k.sum())
    if K > L:
    ...
    self.N = numpy.sum(self.N_k) # N = \sum_{k=1}^K N_k is the total number of uncorrelated configurations pooled across all states
  ...
  def computeExpectations(self, A_kn, output='averages', compute_uncertainty=True, uncertainty_method=None,warning_cutoff=1.0e-10, return_theta=False):
    ...
    if output == 'averages':
      ...
      if compute_uncertainty:
        ...
        for k in range(0,K):
          dA_i[k] = numpy.abs(A_i[k]) * numpy.sqrt(Theta_ij[K+k,K+k] + Theta_ij[k,k] - 2.0 * Theta_ij[k,K+k]) # Eq. 16 of [1]
  ...
  def computeMultipleExpectations(self, A_ikn, u_kn, compute_uncertainty=True, uncertainty_method=None, warning_cutoff=1.0e-10, return_theta = False):
    ...
    # Now that covariances are computed, add the constants back to A_i that were required to enforce positivity
    A_i += (A_min-1)
  ...
  def _computeAsymptoticCovarianceMatrix(self, W, N_k, method=None):
    ...
    if numpy.any(badcolumns):
        ...
        raise ParameterError('Warning: Should have \sum_n W_nk = 1.  Actual column sum for state %d was %f. %d other columns have similar problems' % (firstbad, column_sums[firstbad],numpy.sum(badcolumns)))
    ...
    if numpy.any(badrows):
      ...
      raise ParameterError('Warning: Should have \sum_k N_k W_nk = 1.  Actual row sum for sample %d was %f. %d other rows have similar problems' % (firstbad, row_sums[firstbad],numpy.sum(badrows)))
