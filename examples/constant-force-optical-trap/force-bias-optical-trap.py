...
for k in range(K):
    ...
    g_k[k] = g
    print(
        "timeseries {:d} : g = {:.1f}, {:.0f} uncorrelated samples (of {:d} total samples)".format(
            k + 1, g, int(np.floor(T_k[k] / g)), T_k[k]
        )
    )
...
for i in range(nbins):
    print(
        "{:8.3f} {:8.3f} {:8.3f} {:8.3f} {:8.3f}".format(
            bin_center_i[i], f_i[i], df_i[i], pmf_i[i], bin_width_i[i]
        )
    )
